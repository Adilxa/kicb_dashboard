    ...
    <div className='relative'>
      <Select defaultValue={defaultValue} onValueChange={onChange}>
        <SelectTrigger
          aria-label={label}
          className={clsx(
            'rounded-xl bg-black p-2 text-white transition-colors dark:bg-white dark:text-black',
        ...
        <SelectContent className='min-w-[8rem] overflow-hidden rounded-sm bg-card shadow-md'>
          {items.map(item => (
            <SelectItem
              key={item.value}
              value={item.value}
              className='flex cursor-default items-center px-3 py-2 text-foreground data-[highlighted]:bg-muted'
            >
